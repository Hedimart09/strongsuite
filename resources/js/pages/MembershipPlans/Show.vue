        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4 md:p-6">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground mt-1">
                        {{ plan.subscriptions_count }} active {{ plan.subscriptions_count === 1 ? 'subscription' : 'subscriptions' }}
                    </p>
                    <div class="mt-2">
                        <span
                            :class="getStatusClass(plan.is_active ? 'active' : 'inactive')"
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border p-6">
                    ...
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="mt-1 text-2xl font-bold">{{ formatCurrency(plan.price, plan.currency) }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-muted-foreground">Duration</dt>
                            <dd class="mt-1 text-sm">{{ formatDuration(plan.duration_in_days) }}</dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm">{{ new Date(plan.created_at).toLocaleDateString() }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border p-6">
                    ...
                    <p v-else class="text-sm text-muted-foreground">No features listed</p>
                </div>
            </div>

            <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border p-6">
                <h2 class="text-lg font-semibold mb-4">Recent Subscriptions</h2>
                <div v-if="plan.subscriptions.length > 0" class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-sidebar-border/50">
                            <tr v-for="subscription in plan.subscriptions" :key="subscription.id">
                                <td class="px-4 py-3">
                                    <a :href="`/members/${subscription.member.id}`" class="text-primary hover:text-primary/80">
                                        {{ subscription.member.name }}
                                    </a>
                                    <div class="text-xs text-muted-foreground">{{ subscription.member.member_id }}</div>
                                </td>
                ...
                <p v-else class="text-sm text-muted-foreground">No subscriptions yet</p>
            </div>
        </div>
    </AppLayout>
</template>
